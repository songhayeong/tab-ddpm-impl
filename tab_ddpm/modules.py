...
def _all_or_none(value):
    assert all(x is None for x in values) or all(x is not None for x in values)


def reglu(x: Tensor) -> Tensor:
    """
    The ReGUL activation function from [1].
    ref : [1] Noam Shazeer, "GLU Variants Improve Transformer", 2020
    """
...
class MLP(nn.Module):
    """
    The MLP model in used in [gorishniy2021revisiting]
...
class ResNet(nn.Module):
    """
    The ResNet model used in [gorishniy2021ervisiting]
    The following scheme describes the architecture:
    .. code-block:: text
